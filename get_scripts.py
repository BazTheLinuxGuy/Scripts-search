...
for the program.', prog=program)
    parser.add_argument('-i', '--inputdir',
                        help='Input directory, a "bin" dir.', required=True)
    parser.add_argument('-o', '--outputdir',
                        help='Directory to place results into.', required=True)
...
class Script:
    ...
    def process_filetype(self):
        ...
        if (DEBUG):
            print('**DEBUG:\n'
                  'progname is {}, filetype is {} and size is {}.'.format(self.progname, self.filetype, self.size),end='\n\n')
        self.finfo = fileinfo(self.progname, self.filetype, self.size)
        self.script = self.finfo
...
class Scripts:
    ...
    def writefile(self):
      ...
      #  for j,k in enumerate(self.scripts):
        logdir = args.outputdir
        pat = '/'
        repl = '-'
        sfilename = 'scripts-in'
        direct = args.inputdir
        direct = regex.sub(pat, repl, direct)
        if DEBUG:
            print('\n==> direct =', direct, end='\n\n')
            time.sleep(5)
        sfile = logdir + os.sep + 'scripts-in-' + direct + '.txt'
        if DEBUG:
            print('\n--->** sfile is', sfile, end='\n\n')
    ...
    for filename in files:
        try:
            ...
            for line in p:
                ...
                filetype = line
            p.close()    
            if statinfo.st_mode == stat.S_IFLNK:
                filetype = 'symlink'
            ...
            continue
    if DEBUG:
        print('\n** There are {} scripts in {}.'.format(scripts.length(),
                                                        args.inputdir), end='\n\n')
    scripts.writefile()
    return
